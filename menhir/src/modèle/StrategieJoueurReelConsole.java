...
public class StrategieJoueurReelConsole extends Strategie {
	...
	public void jouerSonTour(Saison saisonActuelle,
		...
		System.out
				.println(this.referenceJoueur.getNom()
		...
		System.out.println("Graines de menhirs en stock : "
				+ this.referenceJoueur.getPaquet().getGrainesDeMenhir());

		System.out
				.println("Quelle carte ingrédient jouez-vous ? (tapez son id) ");
		boolean trouvee = false;
		CarteIngredient carteAJouer = new CarteIngredient(null);
		while (!trouvee) {
			reponse = parametresDePartie.getScanner().next();
			for (Iterator<Carte> it = this.referenceJoueur.getPaquet()
		...
		while (!reponse.contentEquals("engrais")
			...
			if (reponse.contentEquals("engrais")) {
				carteAJouer.utiliser(TypeAction.engrais, this.referenceJoueur,
						this.referenceJoueur, saisonActuelle,
						parametresDePartie);
				System.out.println("Vous avez maintenant "
						+ this.referenceJoueur.getPaquet()
								.getNombreMenhirsAdultes()
			...
			if (reponse.contentEquals("farfadet")) {
				...
				while (!trouvee) {
					reponse = parametresDePartie.getScanner().next();
					for (Iterator<Joueur> it = parametresDePartie
							.getListeJoueurs().iterator(); it.hasNext();) {
			...
			if (reponse.contentEquals("geant")) {
				carteAJouer.utiliser(TypeAction.geantGardient,
						this.referenceJoueur, this.referenceJoueur,
	...
	public int seDefendre(ParametresDePartie parametresDePartie,
		...
		if (parametresDePartie.getTypePartie() == StatutPartie.avancee
			...
			if (!tempCarte.isEstUtilise()) {
				System.out
						.println(acteur.getNom()
								+ " vous lance une attaque de "
								+ puissanceModifie
								+ " "
								+ "graines, Se défendre avec votre carte chien de garde ?"
								+ "\n( " + tempCarte.toString() + " )");
				if (parametresDePartie.getScanner().next().contains("oui")) {
					puissanceModifie = tempCarte.utiliser(destinataire,
							saisonActuelle, puissanceModifie);
					System.out.println(this.referenceJoueur.getNom()
							+ " se défend de " + destinataire.getNom()
							+ " avec ses chiens de garde et ne perd que "
							+ puissanceModifie + " graines de menhir.");
				}
			}
		}
	...
	public void choixDeManche(ParametresDePartie parametresDePartie) {
		System.out
				.println("Voulez-vous une carte Alliee à la place de 2 graines de Menhir ? (oui ou non)");
		if (parametresDePartie.getScanner().next().contains("oui")) {
