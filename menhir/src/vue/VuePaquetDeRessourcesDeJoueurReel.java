...
public class VuePaquetDeRessourcesDeJoueurReel extends VuePaquetDeRessourcesDeJoueur implements Observer {
	...
	public VuePaquetDeRessourcesDeJoueurReel(PaquetDeRessourcesDeJoueur paquetDeRessourcesDeJoueur, Ressources r,
		...
		if (avancee) {
			...
			if (paquetDeRessourcesDeJoueur.getPaquetsDeCartes().get("Cartes Chiens De Garde").get(0) != null) {
				vuesCartes[6] = new VueCarteChiensDeGarde(
						paquetDeRessourcesDeJoueur.getPaquetsDeCartes().get("Cartes Chiens De Garde").get(0), r, 105,
						105, false);
			} else if (paquetDeRessourcesDeJoueur.getPaquetsDeCartes().get("Cartes Taupes Geantes").get(0) != null) {
				vuesCartes[7] = new VueCarteChamp(
						paquetDeRessourcesDeJoueur.getPaquetsDeCartes().get("Cartes Taupes Geantes").get(0), r, 105,
						105, false);
			}
		...
		/*
		 * Set<String> tempKeys =
		 * paquetDeRessourcesDeJoueur.getPaquetsDeCartes().keySet();
		 * for(Iterator<String> it = tempKeys.iterator(); it.hasNext();) {
		 * String tempKey = it.next(); for(Iterator<Carte> yt =
		 * paquetDeRessourcesDeJoueur.getPaquetsDeCartes().get(tempKey).iterator
		 * (); yt.hasNext();) { Carte tempCarte = yt.next(); VueCarte
		 * tempVueCarte = null; if(tempCarte instanceof CarteIngredient) {
		 * tempVueCarte = new VueCarteIngredient(tempCarte); } else if(tempCarte
		 * instanceof CarteChiensDeGarde) { tempVueCarte = new
		 * VueCarteChiensDeGarde(tempCarte); } else if(tempCarte instanceof
		 * CarteTaupesGeantes) { tempVueCarte = new
		 * VueCarteComptageDePoints(tempCarte); } else if(tempCarte instanceof
		 * CarteChamp) { tempVueCarte = new VueCarteChamp(tempCarte); } else {
